...
public final class YmpSettings {
    ...
    public static final String CONTROL_MODE_TOUCH = "touch";
    public static final String CONTROL_MODE_REMOTE = "remote";
    ...
    public static String controlMode(Context context) {
        return normalizeControlMode(prefs(context).getString(KEY_CONTROL_MODE, CONTROL_MODE_AUTO));
    }

    public static void setControlMode(Context context, String mode) {
        prefs(context).edit().putString(KEY_CONTROL_MODE, normalizeControlMode(mode)).apply();
    }

    public static String normalizeControlMode(String mode) {
        String value = mode == null ? "" : mode.trim().toLowerCase(java.util.Locale.US);
        if (CONTROL_MODE_TOUCH.equals(value) || CONTROL_MODE_REMOTE.equals(value)) {
            return value;
        }
