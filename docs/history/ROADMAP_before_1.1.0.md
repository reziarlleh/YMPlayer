# Архив плана до 1.1.0

Исторический снимок на 2026-09-08. Противоречивые статусы ниже отражают этапы
разработки, а не текущее состояние. Актуальный план: [ROADMAP.md](../../ROADMAP.md).

## 1.0.0-beta.13: адаптивный интерфейс

- Выполнены исправление ТВ-плитки, общая нижняя полоса сообщений, учёт системных
  отступов и адаптивные размеры обложки/управления без изменения плеера и SideBar.
- Проверены геометрия интерфейса на API 29/35 и живой интерфейс эмулятора API 35.
- Следующая проверка пользователем: удобство размеров на K4811, ТВ-плитка,
  портрет/альбом на смартфоне, фокус пульта, нижняя полоса в клипах и настройках.
- Beta.12 принята пользователем («Всё Ок»); кэш и рабочую перезагрузку не менять.
- Концепция 2.0 по-прежнему отложена. Это доработка существующей ветки 1.x.

## 1.0.0-beta.12: единая синхронизация избранного

- Реализована одна операция для аудио и обложек с проверкой целостности,
  ремонтом отсутствующих частей и отдельными счётчиками результата.
- На устройстве проверить повторную синхронизацию существующей коллекции
  без лишних загрузок и показ обложек избранного в авиарежиме.
- Структура постоянного кэша сохранена; добавлены соседние checksum-файлы.
- Перезагрузка K4811 в beta.11 подтверждена пользователем. SideBar не менять.

## Уточнение 2026-09-08: SideBar / K4811

- Магнитола пользователя: K4811, а не TS10/TS18; старые записи ниже исторические.
- В 1.0.0-beta.11 добавлен подтверждаемый путь перезагрузки через NWD-службу
  и сохранён отдельный запасной штатный диалог. Подробности: [исследование](../K4811_REBOOT.md).
- Следующая проверка на устройстве: отмена, подтверждение перезагрузки и
  вызов из SideBar поверх CWG. До неё не считать новый путь проверенным на железе.
- Плеер, сон, порядок кнопок и размеры SideBar оставлены без изменения.

## Текущий статус: 1.0.0-beta

Основной набор функций YMPlayer завершён. Ветка `1.0.x-beta` предназначена для
исправления небольших ошибок, обнаруженных длительным тестированием на
смартфонах и автомобильных магнитолах. Следующие значимые версии будут либо
добавлять самостоятельные пользовательские функции, либо адаптировать
приложение к изменениям сервисов Яндекса и Android.

### Возможные следующие направления

- Расширенное управление текущей очередью и большими локальными плейлистами.
- Переименование и редактирование состава плейлистов Яндекс Музыки, если
  используемые операции API останутся стабильными.
- Пагинация поиска и отдельный экран больших результатов.
- "Моя волна" по треку или исполнителю и фильтры настроения после подтверждения
  стабильного формата запросов.
- Дополнительные системные поверхности Android, включая домашний виджет.

Работающий аудиоплеер, кэш понравившихся треков, MediaSession, SideBar и базовую
динамику "Моей волны" следует менять только при подтверждённой необходимости.
...
## Завершено в 1.0.0-beta

- Доработана Волна клипов: бесшовная очередь следующего элемента, перемотка,
  прогресс и отдельная пятсекундная подпись нового клипа.
- Добавлена настройка автоскрытия системных панелей во время клипов.
- Перегруппированы настройки, добавлен экспорт диагностики в `Downloads`.
- Финализированы окно «О приложении» и базовый интерфейс поиска.
- Подготовлена неотладочная release-сборка с обновлением поверх 0.x.
