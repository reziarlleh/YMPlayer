...
История выпущенных версий. Старые записи описывают состояние на момент выпуска;
актуальные возможности и планы находятся в README.md и ROADMAP.md.

## 1.1.0 - 2026-09-08

Первый стабильный выпуск ветки 1.x, без пометки beta.

- Нажатие на статус-бар раскрывает полный текст состояния и журнал: переносы
  строк, прокрутка, высота до 45% доступного экрана. Повторное нажатие сворачивает;
  доступны OK, вверх/вниз и «Назад» с пульта. Чтение журнала выполняется в фоне.
- Волна клипов вновь полноэкранная, без постоянного статус-бара. Сообщения
  отображаются только в скрывающейся панели управления видео.
- Название песни, исполнитель и альбом выровнены в отдельную колонку.
  Лайк/дизлайк/добавление больше не сдвигают название; на больших горизонтальных
  экранах метаданные крупнее, на узких действия располагаются отдельной строкой.
- Обновлены README, текущий аудит и план развития; накопительные старые планы
  перенесены в docs/history. 2.0 и профили по-прежнему отложены.
- Выпуск публикуется как обычный GitHub Release / Latest вместо prerelease.
- Аудиодвижок, алгоритмы кэша, авторизация и рабочие команды SideBar не изменялись.
- Проверка: 94 теста API 29/35, сборка release и lint без ошибок; установка
  поверх beta.13 и управление статус-панелью проверены на эмуляторе Android 15.
- Выпуск: `YMPlayer-v1.1.0-release-b114.apk`, Android 10+.

## 1.0.0-beta.13 - 2026-09-08

Адаптивная компоновка и общая строка состояния.

- Исправлена ТВ-плитка: убран чёрный квадрат вокруг скруглённого логотипа,
  добавлено изображение высокого разрешения с пропорциями баннера 16:9.
- Сообщения занимают две постоянные строки снизу, вне прокрутки: на главной,
  в библиотеке, поиске, окнах настроек и в клипах. Длинный текст не меняет высоту.
- Учтены системные панели, вырез экрана и клавиатура, включая Android 15.
- Размер обложки рассчитывается по доступной области, без прежнего ограничения
  320 dp на горизонтальном экране. Круглые кнопки увеличиваются вместе с доступным
  местом, сохраняя порядок и квадратные области нажатия.
- Выбор источника вынесен над основным содержимым: один ряд на широком экране,
  два ряда на узком. На низком экране убираются дублирующие подписи режима/очереди.
- На очень маленьком экране или при увеличенном системном шрифте содержимое
  можно прокрутить; нижняя строка состояния всегда остаётся на месте.
- Исправлено обновление сводки кэша после её фоновой загрузки.
- Аудиоочередь, кэш, авторизация и функции SideBar не менялись.
- Проверка: 80 тестов на API 29/35, сборка release и lint без ошибок;
  дополнительно интерфейс и обновление APK проверены на эмуляторе Android 15.
  Физическая проверка на магнитоле и ТВ остаётся за пользователем.
- Выпуск: `YMPlayer-v1.0.0-beta.13-release-b113.apk`.

## 1.0.0-beta.12 - 2026-09-08

Единая синхронизация избранной музыки и обложек.

- В настройках оставлена одна кнопка «Синхронизировать избранное».
- За один проход проверяются обе части каждого трека: аудиофайл и обложка.
  Отсутствующие или повреждённые части скачиваются, исправные не загружаются заново.
- Для существующего кэша не требуется очистка или перенос. После проверки
  сохраняются контрольные суммы для обнаружения последующих повреждений.
- Старое аудио проверяется чтением пакетов и сопоставлением длительности;
  новые загрузки дополнительно проверяются по размеру HTTP-ответа.
- Обложки проверяются декодированием с отклонением неполных изображений.
  Лого плеера не записывается в кэш вместо отсутствующей обложки.
- Повреждённые метаданные или отсутствие песни больше не удаляют исправную
  обложку избранного. Снятие лайка по-прежнему удаляет связанные файлы.
- Итоговый статус отдельно показывает музыку, обложки, ошибки и отсутствие
  обложки у источника. Повторное нажатие докачивает оставшееся.
- SideBar не изменён; пользователь подтвердил работу перезагрузки beta.11.
- Выпуск: `YMPlayer-v1.0.0-beta.12-release-b112.apk`.

## 1.0.0-beta.11 - 2026-09-08

Переработана перезагрузка магнитолы K4811 из SideBar.

- Кнопка открывает окно подтверждения с подключением к штатной NWD-службе.
- Запрос отправляется один раз, только после подтверждения пользователя;
  недоступность службы и отсутствие ответа отображаются явно.
- Сохранён отдельный запасной запуск штатного диалога перезагрузки.
- Удалены ненадёжные launcher-broadcast запросы с ложным статусом успеха.
- Убрано ошибочное обозначение TS18 в сообщении о сне и актуальном README.
- Сон, порядок/размеры кнопок, аудио- и видеоплеер не менялись.
- Выпуск: `YMPlayer-v1.0.0-beta.11-release-b111.apk`.
- Физическую перезагрузку через новый путь предстоит подтвердить на K4811.
